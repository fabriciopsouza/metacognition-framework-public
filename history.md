...
## 2026-09-23 — v1.91.0: cinco sessões viram uma, e o site público era servido pela fonte privada

O release fechou **36 dias** de trabalho que não lançava: três seções não lançadas empilhadas com
títulos inconsistentes (`[Nao lancado]` duas vezes, `[Não liberado]` uma), 475 linhas, cobrindo
ADR-010, 011, 045, 051, 092, 104 e 108 a 115. Consolidadas numa versão só: **3.034 → 3.066 linhas**,
`+35 / -3` no diff. As 3 remoções são os 3 cabeçalhos antigos, cada um substituído 1:1 por um `###`;
nenhum bloco de conteúdo saiu. *(Eu havia escrito "3.034 → 3.064, +30" — errado por 2, porque medi
antes das correções da própria revisão. O QA pegou na 3ª rodada.)*

**Três PRs entraram:** [#142](https://github.com/fabriciopsouza/metacognition-framework-public/pull/142)
(execution-report represado desde 21/09), [#143](https://github.com/fabriciopsouza/metacognition-framework-public/pull/143)
(canário de workflows + §3.2 RUNBOOK) e [#144](https://github.com/fabriciopsouza/metacognition-framework-public/pull/144)
(a fixture da união), mergeado como `f64cbd4`.

**A unificação das sessões foi o trabalho, não o preâmbulo.** Havia **5 sessões vivas na mesma
árvore**, três escrevendo. O dono precisou cobrar **duas vezes** — e a segunda com razão, porque
nós estávamos *coordenando* ordem de PR quando ele havia pedido *unificação*. Coordenação não é
unificação. Duas eram de domínio (CAPA, budget-ti) e saíram do escopo por régua dele: unificar quem
**melhora** o framework, não quem o **usa**. As duas de núcleo entregaram e pararam.

**O custo real da árvore compartilhada, medido:** um `reset --hard origin/main` de alguma sessão
destruiu as 47 linhas da §3.2 RUNBOOK de outra, na mesma manhã. O `git reflog` registra o evento
(`reset: moving to origin/main`, 2026-09-22 17:13:35 — **cito a data, não o índice, porque índice de
reflog desliza a cada commit**), mas ele **não prova a perda de conteúdo**: o próprio commit que
restaurou a seção diz que conteúdo nunca commitado não tem reflog, nem stash, nem fsck. A prova da
perda é o relato da sessão autora, que ainda tinha o diff em contexto — e é por isso que ela é uma
prova frágil, não uma prova mecânica. A
recuperação só foi possível porque a sessão autora ainda tinha o diff em contexto — reflog não
alcança o que nunca virou commit. O que preservou o resto foi `git commit -- <caminho>`, que monta
índice temporário só daquele path.

**Três sessões afirmaram três coisas diferentes sobre o mesmo `trabalhos.py --force`, e nenhuma
acertou inteira.** Uma testou e disse "preserva"; outra sofreu a perda e disse "reescreve tudo"; eu
li o código e disse "preserva". A medição final: seção `##` não-canônica **sem** passar a flag
sobrevive; **com** a flag é destruída; conteúdo antes do primeiro título canônico é destruído; e
chave de frontmatter fora das 6 de `CAMPOS` é destruída. A causa é uma só — **`_escrever` não é
simétrico a `_ler`**: o leitor aceita mais do que o escritor sabe regravar, e a diferença some com
exit 0.

**O achado do gate, que se resolveu sozinho no meio do caminho.** No sha `4584462` o CI deu
`ubuntu 36/37` com a mutação `união vira any` **sobrevivendo**, enquanto Windows e macOS davam
37/37 — mesmo código, resultado oposto por plataforma. O conserto foi na **fixture**, não na regra
do ADR-115: ela consultava o git por caminho. Validado nas três plataformas em `f64cbd4`.

**E o incidente que inverteu a pauta.** O repositório-fonte estava **público**, com **36 arquivos
rastreados** citando SUA-ORG, um cliente regulado, Agidesk e Thanner — **25 deles fora de `docs/_private/`**,
incluindo `CHANGELOG.md`, `bootstrap.ps1` e vereditos de QA. Entre eles,
`INCIDENTE-VAZAMENTO-2026-05-31.md`, que se descreve como *"PRIVADO · NÃO DISTRIBUIR"* e *"a chave
para reverter"* a anonimização. Fechado no mesmo bloco, com `forks=0` — ninguém copiou.

**A arquitetura já estava errada antes disso, e ninguém tinha visto:** o site público era servido
pelo Pages da **fonte privada**, não do export limpo. Havia duas configurações de site — uma ativa
servindo conteúdo não filtrado, e uma dormente no `metacognition-framework-public`, desligada.

**E aqui o release quase publicou uma mentira sobre si mesmo.** Eu ativei a dormente com um
`workflow run` e declarei o incidente fechado, escrevendo que o domínio antigo estava "desativado".
Não estava. A **3ª rodada do QA testou a URL ao vivo** e trouxe `HTTP 200`, com rebuild do dia
anterior. A causa: **tornar o repositório privado não desativa o GitHub Pages dele** — a API
mostrava `public=true` num repo `private=true`, e o site seguia servindo a `guia/web/` da fonte.
Dimensionando com honestidade: o `pages.yml` publica **só** `guia/web/`, que tem **um** arquivo sem
nome de cliente nenhum — então não era vazamento de dado, era uma landing page duplicada com 32
links que passaram a dar 404 quando o repo fechou. O Pages da fonte foi **removido neste bloco**
(API devolve 404 agora); o `200` residual é cache de CDN. Site em
`https://fabriciopsouza.github.io/metacognition-framework-public/`, verificado: HTTP 200, 37.584
bytes e **zero** ocorrências de nome de cliente. Dos 36 links reais da página (fora as 11 âncoras
internas), 32 vão para `github.com` (o repositório), 1 para `code.claude.com`, 1 é relativo, e **2 apontavam
para o domínio do site antigo**: os botões de compartilhar do LinkedIn e do Facebook, percent-encodados
— e é por serem percent-encodados que escapavam da reescrita do exportador, que só casa a forma plana.
Um terceiro, o botão de copiar link do Instagram, tinha o mesmo defeito num `onclick`, fora da contagem
de `href`. **Os 3 foram consertados neste release**, na fonte `guia/web/index.html`. O exportador não
estava errado: o docstring de `rewrite_repo_urls` declara que o site fica em `-public`; a fonte é que
contrariava a própria doutrina.

**O que o `export-clean.py` provou:** SUA-ORG, um cliente regulado e Thanner dão **zero** nas distribuições
públicas, embora existam na fonte. Não era suposição — foi medido nos repos-sombra.

**Quatro pendências fechadas** (`adr-113`, `canario-para-workflows`, `uniao-por-caminho`,
`skill-service-valuation` atualizada e mantida aberta com resíduo). Três delas estavam **entregues
e ainda marcadas como abertas** — a terceira ocorrência do mesmo defeito no mesmo dia: abrir
pendência é mecanizado, fechar depende de alguém lembrar.

**O que fica aberto, declarado:** ~35 pendências, sendo ~18 do núcleo. As prioritárias continuam
sendo `enforcement-que-viaja` (o squad e o QA adversarial não são determinísticos fora deste repo)
e `escopo-de-repo-da-sessao`.


## 2026-09-22 — O bloco represado sai, e o gate para de medir o que não muda (ADR-113/114/115)

Três ADRs que não commitavam havia semanas entraram na `main` pelo [PR #139](https://github.com/fabriciopsouza/metacognition-framework-public/pull/139):
137 arquivos, 22.649 inserções, `ed087b5..5305ae2`. Não commitavam porque o `squad_gate` —
corretamente — não tinha evidência que os cobrisse. Foram 39 rodadas adversariais no total, três
delas nesta sessão, e o bloco saiu **sem override**.

**O que destravou não foi uma exceção, foi um conserto.** O `_recente_o_bastante` media
ancestralidade de *commit*, e o gate roda no pré-commit — quando, por definição, todo arquivo do
bloco está fora do HEAD. Medido: três vereditos aprovativos de **agosto** seguiam "recentes" para
arquivos editados em **setembro**. O veredito passou a carregar `blobs_revisados`, o hash do blob
indexado, que é a única coisa que descreve conteúdo ainda não commitado.

**E a regra que fabricava o artefato que ela mesma criticava.** `_qa_critic_attested` exigia UM
veredito cobrindo TODOS os caminhos: para fechar um bloco de 91 era preciso declarar os 91 num
artefato só, e nenhum crítico sozinho revisou 91 arquivos. Medido: **57 dos 91** dependiam desse
veredito-cobertor. Agora vale a união por caminho.

**O achado que vale mais que os dois consertos** veio contra mim, na 37ª: ao otimizar o caminho
novo eu pulei a validação de `sha_revisado`, raciocinando que o hash de conteúdo é mais forte que
a ancestralidade. Verdade para *aquela função* e falso para o sistema — `sha_revisado` também é a
chave que `_reprovacao_vigente` usa para achar a reprovação irmã, e um aprovativo com sha podre
fazia uma reprovação legítima deixar de ser vigente. Era a reabertura do defeito que as rodadas 8,
9 e 10 fecharam. *Antes de pular a validação de um campo, procurar quem mais o lê.*

**A cegueira que a CI expôs, e que nenhuma das 38 rodadas anteriores podia ver:** o gate roda numa
plataforma só. Os 20 pinos de hash foram calculados de bytes CRLF e o repo guarda LF — o canário
de integridade só podia passar no Windows. E um canário montava barreira de escrita com semântica
NTFS, reprovando no POSIX uma garantia que existe. O trade-off "1 OS" do ADR-088 segue defensável;
o que faltava era medir o custo dele em vez de citá-lo.

**Padrão da sessão:** três rodadas seguidas tiveram o achado principal na **prosa**, não no código
— um comentário afirmando critério de aceite falso sobre o próprio arquivo, a palavra "mais
estrito" onde cabia "mais cirúrgico", e o gancho do índice de memória que envelheceu enquanto o
corpo era atualizado. Nenhum canário lê prosa.

**Seis sessões escreveram nesta árvore durante o bloco.** O HEAD andou quatro vezes. Duas sessões
atribuíram commits uma à outra, e nenhuma das duas era responsável. O que impediu estrago não foi
gate — foi releitura antes de editar, e uma sessão ter parado para perguntar antes de commitar.

**Próximo passo:** decidir o desenho para a composição de vereditos de SHAs diferentes (três vias
na mesa, custa um ADR) e fechar o canário de `.agent/workflows/*.md`, que é o buraco com mais
reincidência medida.

## 2026-08-18 — Release v1.90.0 (MINOR): o boletim deixa de ser escrito só pelo avaliado (ADR-108)

O dono fez a pergunta que faltava: depois de zerar o passivo, dá para dizer que é determinístico
e que não é teatro? Não dava — e o motivo não era o número de capacidades provadas. Era que **eu
escrevia o boletim que o gate lia**.

O método dele estava certo e rodando: crítico isolado, modelo diferente, quatro rodadas achando
defeito real. O furo era outro — independência de **julgamento** havia; procedência do **registro**
não. `agentId` e `modelo` eram strings que eu digitava.

**A correção usa dado que eu não produzo:** os transcripts do harness. O veredito passa a carregar
o sha256 do relatório que o crítico devolveu, e o verificador procura esse hash no log. Dezessete
dos cinquenta e um vereditos existentes foram verificados retroativamente contra o relatório real;
os trinta e quatro restantes ficam listados como legados, porque fabricar procedência para
veredito antigo seria o teatro que o mecanismo combate.

**E a primeira versão do detector acusou fraude num veredito honesto** — leu a frase "por que não
é REPROVAR" como se fosse a conclusão. Vale registrar porque a lição é assimétrica: aqui o
falso-positivo custa mais que o falso-negativo, já que um detector que acusa o inocente é
desligado, e aí não resta nem o falso-negativo.

**O que continua aberto, e está escrito:** qual modelo de fato respondeu não é verificável (o
harness não grava os turnos do subagente); amaciamento fino de severidade escapa; o `escopo_paths`
segue escolhido por mim; e o prompt dado ao crítico também — crítico independente apontado para a
pergunta errada é independente e cego.

**Próximo passo:** amarrar o verificador ao `squad_gate` como bloqueio, hoje ele é canário da
suíte. Depois, o furo irmão: validar `escopo_paths` contra o que o crítico de fato leu.

## 2026-08-18 — Release v1.89.0 (MINOR): o passivo do registro acabou (ADR-106 fechado)

Das 90 capacidades, **52 têm sabotagem provada** e nenhuma declara bloquear falha sem prova.
As 35 que sequer diziam como se protegiam foram classificadas pelo que o código faz — e a
maioria honestamente **não** bloqueia: avisa, registra, ou pede confirmação humana.

**Classificar foi o trabalho; declarar teria sido fácil e falso.** O risco era rotular para
baixo e escapar da exigência de prova, então o crítico varreu as 38 não-`fail-closed` atrás de
`decision: deny` e `sys.exit(2)` — zero ocorrências. Duas capacidades só viraram prováveis
depois de consertar o canário delas, e uma terceira (`skill-contract`) descobriu-se que tinha
um validador que **ninguém nunca executou**.

**A lição que se repetiu quatro vezes, incluindo comigo:** o teste que confirma o que já se
acredita nasce verde e vazio. A fixture que escrevi para provar a contagem da vitrine não
distinguia "tem o campo mutação" de "tem mutação válida", e o crítico demonstrou reescrevendo a
função sem a validação — mesmo resultado. Depois, o fix de CRLF que eu tinha acabado de fazer
não estava guardado por canário nenhum. Em ambos, quem separou "funciona" de "está guardado"
foi a reversão feita numa cópia, não a leitura do diff.

**Um gate também falha por não deixar passar.** A regra que barra arquivo sem histórico tornava
todo arquivo novo permanentemente não-commitável, empurrando para `--no-verify` — mais perigoso
que o buraco que ela fecha.

**Próximo passo:** os follow-ups declarados no ADR-106 — `--provar` que retorna 0 com
capacidades puladas, `escopo_paths` sem validação contra o que o crítico revisou, e a âncora por
HEAD que não cobre edição entre veredito e commit. Nenhum é urgente; todos estão escritos.

## 2026-08-17 — Release v1.88.0 (MINOR): nenhuma capacidade declara bloquear sem prova (ADR-106)

A classe "declara `fail-closed` e nunca foi sabotada para confirmar" chegou a zero. Eram 22 no
censo original do ADR-106; hoje são zero, e 29 capacidades têm sabotagem provada de verdade.

**O passivo não era homogêneo, e essa foi a descoberta que orientou o bloco.** Nove capacidades
tinham mecanismo e canário em arquivos distintos — bastava declarar a mutação. As outras oito
tinham `mechanism == test`: o gate *era* o próprio canário, e sabotar a detecção o deixava
**verde**. O auditor recusava a prova, corretamente. Não era desleixo acumulado; era impossível
provar do jeito que estavam. Cada uma ganhou uma fixture sintética que é obrigada a acertar.

**O que mais ensina não é o resultado, é o caminho.** O canário que escrevi para prender uma
correção do próprio auditor nasceu verde-sem-significado **três vezes** — e a terceira só caiu
porque o crítico independente mostrou que `encoding=None` passaria. Três verdes vazios dentro do
mecanismo criado para acabar com verdes vazios. É a medida de quão fácil é escrever o teste que
confirma o que já se acredita, mesmo sabendo disso.

**Um gate pode falhar por não poder passar.** A proteção da `main` exigia um check que nenhum job
produzia: a matriz rodava verde nos três sistemas e nenhum PR era mergeável. Ficou meses
invisível porque merge bloqueado *parece* rigor.

**Próximo passo:** classificar as 34 capacidades sem o campo `enforcement` pela leitura do que o
canário de cada uma faz de fato — e não marcar nenhuma como `fail-closed` sem fixture, sob pena
de recriar o passivo recém-zerado. Antes disso, mecanizar a detecção de trecho `de` ambíguo no
auditor, que é barato e fecha a única dívida aberta desta classe.

## 2026-08-17 — Release v1.87.0 (MINOR): o silêncio das distribuições vira número (ADR-049)

Fecha o único ponto de falha silencioso que restava. As cinco distribuições são geradas do
canônico por um workflow do GitHub Actions; quando ele esteve desligado por billing, elas
**congelaram por vinte releases sem nenhum alarme**, e a deriva só apareceu quando alguém foi
olhar à mão, meses depois. O workflow voltou a funcionar — medido nesta sessão —, mas o risco é
estrutural: se cair de novo, o silêncio volta igual.

`tools/shadow_drift.py` mede a distância em commits, lendo o SHA que o próprio workflow grava na
mensagem de publicação de cada distribuição. Melhor que comparar versão: pega também o caso em que
o export rodou e falhou no meio.

**A decisão que mais importa:** sem rede ou sem credencial, ele declara `SKIP` e diz por quê, em
vez de reportar "em dia". Foi exatamente assim que a deriva passou despercebida da primeira vez —
ausência de medição lida como aprovação. O canário protege essa propriedade de forma explícita: se
o `SKIP` virar aprovação, ele fica vermelho. E não reprova por falta de internet, porque
verificador que trava offline é desligado na primeira viagem.

**O canário de vitrine funcionou como projetado, na mesma sessão em que nasceu.** Ao registrar a
capacidade nova, os números da página ficaram defasados e a suíte caiu na hora. Corrigi e passou.
É a primeira vez que o mecanismo cobra o próprio autor.

**Próximo passo:** o passivo — dezessete capacidades que declaram bloquear falha e nunca foram
provadas, e trinta e quatro que não declaram como se protegem. Volume, com método já validado em
doze migrações.

## 2026-08-17 — Release v1.86.0 (MINOR): a vitrine promete menos, prova mais, e não pode mais envelhecer mentindo

**Correção de premissa antes do trabalho.** O dono pediu para atualizar todas as distribuições
relacionadas, dizendo que deveria ser automático. Eu tinha registrado que as cinco sombras estavam
congeladas porque o GitHub Actions morreu por billing — **fui medir e não é mais verdade**. O
workflow rodou três vezes no dia, todas com sucesso; as cinco estão no commit da v1.85.0, com as
ferramentas novas presentes e o cofre `docs/_private/` ausente (404). Já era automático e já tinha
funcionado. A memória foi corrigida, porque teria feito a próxima sessão refazer trabalho
desnecessário. **O ponto único de falha continua real:** se o Actions cair de novo, as sombras
envelhecem sem alarme — não existe canário comparando a versão delas com a do canônico. Candidato
a próximo bloco.

**A vitrine.** Ela já era boa, e a parte difícil ela acertava: tinha uma seção sobre o que os gates
**não** resolvem, coisa que quase ninguém escreve. O defeito era outro — prometia em adjetivos onde
podia provar em número. A abertura empilhava quatro promessas não-verificáveis; virou uma ideia só,
*onde verde significa alguma coisa*, que é a tese real do repositório.

Seção nova com os dois lados no mesmo destaque visual: o verificado (38 de 89 provadas, 77
canários, zero pendências) e o não-verificado (51 desconhecidas, o teto de auto-atestação, o que
não se mecaniza). Sem exagero e sem o inverso — modéstia performática é sicofância ao contrário.

**O mecanismo que impede a página de virar propaganda.** Número em site de divulgação envelhece
calado, e propaganda desmentida pelo próprio repositório é pior que número nenhum. O canário de
vitrine passa a derivar os três números das mesmas fontes que o auditor usa e a exigir que a página
os afirme. Provado plantando um número falso: fica vermelho.

**Próximo passo:** o passivo de 51 capacidades — volume, não descoberta. E, como candidato de menor
esforço e bom retorno, o canário de deriva das sombras: comparar a versão publicada em cada uma
com a do canônico, para que o ponto único de falha deixe de ser silencioso.
...
- **Pedido novo chegando no meio do trabalho deve ir para uma FILA ESCRITA, nao para a memoria do
  agente** (aberto em 21/08/2026, pelo dono: *"nao pare de fazer o que esta fazendo. adicione os
  pedidos na fila -> este tb deve ser um comportamento PADRAO do framework ou qualquer ferramenta
  relacionada"*).
  **O que falta.** O ADR-112 ja resolve METADE: nao abandonar trabalho em curso. A outra metade nao
  existe — o pedido que chega no meio precisa ficar REGISTRADO em arquivo no momento em que chega.
  Hoje ele fica na cabeca do agente ate o fim do bloco, e cabeca de agente e' o unico lugar do
  sistema sem canario: a sessao compacta, o contexto vira resumo, e o pedido some sem erro e sem
  aviso. Ja aconteceu nesta sessao — o dono precisou repetir "Um comando NAO anula o outro" e
  "Qd peco uma coisa, NAO ELIMINA o que pedi antes".
  **Escopo:** comportamento padrao do framework, nao de um projeto. Gravar na chegada, sem
  interromper; devolver a fila ao dono no fecho do bloco; e um canario que prove que pedido
  registrado nao evapora entre um bloco e o seguinte.
  **Nao confundir com** o item de enforcement abaixo: aquele e' sobre o squad nao viajar para
  projeto novo; este e' sobre o pedido do dono nao sobreviver dentro da mesma sessao.

- **REFORÇADO PELO DONO EM 21/08/2026, terceira vez:** *"este deve ser o comportamento em TODOS
  os projetos. qd eu mandar atualizar ou iniciar nova sessao, ativar a skill/ capacidade projetos
  agidesk, etc, o repo em trabalho deve atuar exatamente como o framework pede e como as
  capacidades invocadas pedem"*. **Escopo que isto acrescenta ao item abaixo:** não basta o
  enforcement (hooks, gates, squad) viajar — a ATIVAÇÃO das capacidades declaradas pelo projeto
  também tem de ser automática no boot e no "atualizar", em vez de depender de o agente lembrar.
  Hoje o `projeto.json` de um projeto já declara o que ele usa (`adotado`, `adaptadores`); falta o
  boot LER essa declaração e se comportar de acordo. Medido em 21/08/2026: `PROJETO-QUE-ADOTOU` não tinha
  `.agent/`, `.claude/`, nem uma menção ao framework — ele foi aplicado por sessão, nunca instalado.

- **PRIORIDADE MÁXIMA DO PRÓXIMO BLOCO — o squad e a crítica adversarial não são determinísticos
  fora do repo do framework** (aberto em 18/08/2026, por reclamação do dono; **não é a primeira vez
  que ele reclama exatamente disto** — o gatilho declarado no ADR-097 item 4 está atingido duas vezes:
  o dono declarou E o passo desapareceu sem ninguém notar, por horas).
  **O que aconteceu, medido em 18/08/2026 na abertura do projeto `PROJETO-QUE-ADOTOU`:** o conjunto
  documental inteiro foi escrito, commitado três vezes e declarado pronto **sem uma única rodada de
  crítica adversarial em modelo isolado**. Pior: o autor ofereceu *autocrítica* no lugar, e o dono
  precisou dizer que nunca pediu autocrítica — é sempre adversarial. Substituir o crítico
  independente por autocrítica é exatamente o defeito que o ADR-108 fechou do outro lado (o avaliado
  escrevendo o próprio boletim), reaparecendo por outra porta.
  **As quatro causas, todas verificadas por comando, não supostas:**
  **(1) A aplicação é por repositório e o repositório novo nasceu sem ela.**
  `PROJETO-QUE-ADOTOU/.git/hooks/pre-commit` **não existe**; `tools/install_git_hooks.py` existe e ninguém
  o chamou na criação do projeto. O `.claude/settings.json` do framework (route-gate, mission-gate)
  não vale em outra pasta — então nenhum gate de boot ou de turno rodou lá.
  **(2) O gate é de COMMIT, e o que precisa ser barrado é a ENTREGA.** O `pre-commit` bloqueia
  `git commit`; ele não tem como impedir o agente de dizer "pronto" no chat, que é onde a entrega
  acontece de fato. Um gate que só vive no commit é cego para o canal principal.
  **(3) O `squad_gate` depende de `behaviors/manifest.json`, que só existe no núcleo.** Projeto de
  domínio não tem manifesto, então não há o que cobrar — o gate passaria mesmo instalado.
  **(4) A CAUSA NOVA, e a mais grave porque é invisível: o runtime pode VETAR o subagente e o
  framework não detecta o veto.** A sessão rodou com instrução de não disparar subagente sem pedido
  explícito do usuário. O framework declara o crítico isolado como obrigatório, o runtime proibiu, e
  o efeito foi o passo obrigatório **desaparecer em silêncio** — o agente seguiu trabalhando e
  pedindo autorização em vez de falhar ruidosamente. Enquanto o framework não distinguir
  "não rodou porque passou" de "não rodou porque foi proibido", o mecanismo é opcional na prática.
  **O que o dono pediu, e é o veículo da correção:** empacotar o framework como **plugin instalável
  a partir de repositório** — um stack que o time da SUA-ORG configura UMA vez e que traz junto, em
  sequência: skills, subagentes com `model:` fixado (crítico != autor por definição, não por
  lembrança), hooks de boot/turno/entrega, e comandos. **Verificado em 18/08/2026: não existe.**
  Não há `.claude-plugin/` nem `marketplace.json` no repo — hoje a instalação é manual, por guia.
  **Candidatos de mecanismo, a decidir por ADR no próximo bloco (nenhum implementado):**
  (a) gate de ENTREGA, não de commit — o bloco não fecha sem veredito de crítico isolado no ledger;
  (b) semeadura de projeto novo: um comando único que instala hooks + settings + manifesto mínimo ao
  criar projeto de domínio, para o enforcement viajar com o trabalho em vez de ficar no núcleo;
  (c) **fail-closed no veto**: se o subagente não pode ser disparado, o framework anuncia e trava,
  em vez de continuar; o veto vira registro, não silêncio;
  (d) empacotamento como plugin, com o subagente crítico declarando `model:` diferente do autor.
  **Critério de aceite, binário (EMENDADO em 19/08/2026 por achado do QA de entrega):** abrir um
  projeto novo de domínio numa pasta virgem, o crítico do **degrau abaixo** rodar **sem ninguém pedir**
  nas junções, **e o crítico do degrau acima rodar na entrega quando autorizado** — e a ausência dele, quando não houver autorização, aparecer **declarada** no veredito em vez de silenciosa (ADR-110; achado MÉDIA do QA de junção de 19/08, que pegou este critério exigindo incondicionalmente o degrau caro e portanto impossível de fechar sem violar a política que ele mesmo deveria cobrar) — a versão anterior falava de "a crítica
  adversarial" no singular, e podia ser quitada com um crítico só, deixando o segundo morrer sem
  cobrança — e, se o runtime proibir, o bloco
  falhar visivelmente em vez de seguir. Enquanto isso não passar, esta pendência não fecha.
  **Rastro desta ocorrência:** o projeto em `PROJETO-QUE-ADOTOU` (3 commits sem veredito) e o registro em
  `~/.claude/trabalhos/PROJETO-QUE-ADOTOU.md`.
