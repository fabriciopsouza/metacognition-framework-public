...
- Status: **Proposto** · Data: 2026-06-05 · Decisores: dono + squad (architect)
...
`tools/framework_onboarding.py` (`is_framework_repo`/`needs_onboarding`/`mark_onboarded`, CLI `--check`/`--mark`); `tools/test_framework_onboarding.py` (detecta fonte, ignora projeto, 1× idempotente); `.agent/workflows/start-session.md` step 0 (doutrina do popup). **DONE quando:** abrir o repo-framework 1ª vez → popup com link + opções; "usar" instala global e orienta a abrir o projeto; não re-pergunta. Status→Aceito após verificação.
